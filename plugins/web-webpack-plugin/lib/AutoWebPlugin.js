...
const initconfig = require('../../../build/command');
...
function getDirsInDir(dir, ignorePages = []) {
	...
	var pageArr = initconfig.getUserConfig();

	if(process.env.FILE_NODE != undefined){
		console.log(process.env.FILE_PATH);
		var fileNode = JSON.parse(process.env.FILE_NODE);
		if(process.env.BUILD_ALL!=undefined && process.env.BUILD_ALL){
			pageArr =  fileNode["files"];
		}
	}
	var resultArr = [];
	for (var i = 0; i < pageArr.length; i++) {
		if (files.indexOf(pageArr[i]) == -1) {
			console.log("页面信息配置异常:", pageArr[i]);
		}else{
			resultArr.push(pageArr[i]);
		}
	}
	return resultArr;
...
class AutoWebPlugin {
	...
	constructor(pageDir, options) {
		...
		this.entryMap = {};
		this.webpackEntry = {};
		const { template, templateCompiler, entry, filename, ignorePages, preEntrys, postEntrys, } = options;
